...
int CamShiftDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output)
{
  ...
  if(!bp_init_ok)
  {
    ...
    set_roi(I, cv::Rect(sx/2-50,sy/3,20,20));//Rect(100,300,50,50));
  }
  if(bp_init_ok)
  {
    cv::MatND backproj;
    ...
    cvtColor(backproj, output.images[1], cv::COLOR_GRAY2BGR);
    ...
    {
      cv::rectangle(I, trackwindow, cv::Scalar(255,255,0), 3);
...
int SousArrierePlanDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output)
{
 ...
 cv::Mat tmp, mask;
  ...
  output.images[0] = I;
  resize(I,tmp,cv::Size(0,0),0.25,0.25);
  algo->apply(tmp, mask);
  ...
  cv::Mat O = I.clone();
  cvtColor(I, O, cv::COLOR_BGR2GRAY);
  ...
  //output.images[1] = cv::Mat::zeros(O.size(), CV_32F);
...
# ifdef OCV240
  ...
  images[1] = tmp.clone();
  for(auto r: brects)
    cv::rectangle(images[1], r, cv::Scalar(0,255,0));
  resize(images[1], images[1], cv::Size(0,0), 4, 4);
...
OptFlowDemo::OptFlowDemo()
{
  ...
  reset = true;
  algo = cv::optflow::createOptFlow_DualTVL1();
...
int OptFlowDemo::proceed(OCVDemoItemInput &input, OCVDemoItemOutput &output)
{
  ...
  {
    reset = false;
    cv::cvtColor(I, Iprec, cv::COLOR_BGR2GRAY);
    output.images[1] = I;
    //out.images[0] = cv::Mat::zeros(I.size(), CV_8UC3);
    return 0;
  }

  cv::cvtColor(I, I1, cv::COLOR_BGR2GRAY);
  ...
  normalize(mag,nmag,0,1.0,cv::NORM_MINMAX);
  _hsv[0] = angle * ((360.0 / (2 * 3.141592))); // Teinte (couleur) = angle
  _hsv[1] = 1.0 * cv::Mat::ones(angle.size(), CV_32F); // Chromaticité = 1
  _hsv[2] = nmag; // Luminance
  merge(_hsv, 3, hsv);
  output.images[1] = cv::Mat(hsv.size(), CV_8UC3);
